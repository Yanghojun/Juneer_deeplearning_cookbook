...
def plot_dist(X1,X2,label1,label2,save_dir):
    ...
    Args:
        X1 ([type]): Score of normal
        X2 ([type]): Score of abnormal
    ...
    #log scale figure
    f_log=plt.figure(figsize=(30,20))
...
def save_pair_fig(input,output,code,save_path):
    '''
    ...
    :return:
    '''
    save_ts_heatmap(input,output,code,save_path)
...
def save_ts_heatmap(input,output,code,save_path):
    x_points = np.arange(input.shape[1])
    
    # fig: 전체 그래프를 뜻함. ax: 배열이며 ax[0]이면 첫번째 그래프를 뜻한다
    fig, ax = plt.subplots(2, 1, sharex=True,figsize=(30, 20),gridspec_kw = {'height_ratios':[6,1]})
    sig_in = input[0, :]
    sig_out = output[0, :]
    sig_code = code
    ax[0].plot(x_points,sig_in,'b',linewidth=1.5,label="input signal")
    ax[0].plot(x_points,sig_out,'black',linewidth=1.5,label="output signal")
    ax[0].scatter(np.where(code != 0)[0], sig_in[np.where(code != 0)[0]], s=100, color='red', marker='o', label='Labeled as wrong')
    ax[0].set_yticks([])
